<template>
    <div class="container mt-4 background-verdeopaco rounded-3 p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="">Promociones</h5>
            <button class="btn btn-success" @click="abrirFormulario()">+ Nueva promoción</button>
        </div>

        <div v-if="loading" class="">Cargando...</div>

        <div v-else>
            <PromoTable :datos="promociones" :campos-filtrables="['name', 'discount_percentage']" :por-pagina="20"
                :exportable="true">
                <template #thead>
                    <th>Nombre</th>
                    <th>Descripción</th>
                ...
                <template #row="{ item }">
                    <tr @click="abrirFormulario(item)" class="cursor-pointer">
                        ...
                        <td>
                            ...
                            <button class="btn btn-sm btn-outline-danger" @click.stop="eliminarPromocion(item.id)">
                                <i class="bi bi-trash" />
                            </button>
                        </td>
                    </tr>
...
import PromoTable from '../../components/Table.vue'
...
const obtenerPromociones = async () => {
    loading.value = true
    try {
        const res = await PromocionService.getPromotions()
        promociones.value = Array.isArray(res.data.data) ? res.data.data : []
...
const guardarPromocion = async () => {
    ...
    try {
        if (form.value.id) {
            await PromocionService.patchPromotion(form.value.id, payload)
            showSuccess('Actualizado correctamente')
        } else {
            await PromocionService.postPromotion(payload)
...
const eliminarPromocion = async (id) => {
    ...
    try {
        await PromocionService.deletePromotion(id)
        await obtenerPromociones()
