<template>
    <div class="container mt-4 background-verdeopaco rounded-3 p-4">
        ...
        <div v-else>
            <AdminTable :datos="destinos" :campos-filtrables="['name', 'description']" :por-pagina="20"
                :exportable="true">
                <template #thead>
                    <th>Nombre</th>
                    <th>Descripción</th>
                ...
                <template #row="{ item }">
                    <tr @click="abrirFormulario(item)" class="cursor-pointer">
                        ...
                        <td>
                            ...
                            <button class="btn btn-sm btn-outline-danger" @click.stop="eliminarDestino(item.id)">
                                <i class="bi bi-trash" />
                            </button>
                        </td>
                    </tr>
...
const obtenerDestinos = async () => {
    ...
    try {
        ...
        destinos.value = Array.isArray(res.data) ? res.data : []
    } catch (err) {
        console.error(err)
    } finally {
        loading.value = false
...
const eliminarDestino = async (id) => {
    ...
    try {
        await DestinoService.deleteDestino(id)
        await obtenerDestinos()
