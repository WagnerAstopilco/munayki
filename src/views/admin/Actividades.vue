<template>
  <div class="container mt-4 background-verdeopaco rounded-3 p-4">
    ...
    <div v-else>
      <AdminTable :datos="actividades" :campos-filtrables="['name', 'description']" :por-pagina="20" :exportable="true">
        <template #thead>
          <th>Nombre</th>
          <th>Descripción</th>
          <th>Acciones</th>
    ...
    <AdminModal :show="mostrarModal" :on-close="cerrarModal">
      ...
      <form @submit.prevent="guardarActividad">
        ...
        <div class="mb-2">
          ...
          <Multiselect v-model="form.product_ids" :options="productos" :track-by="'id'" :label="'name'" :multiple="true"
            placeholder="Selecciona los productos" :allow-empty="true" />
        </div>
...
const abrirFormulario = (actividad = null) => {
  form.value = actividad
    ? { ...actividad }
    : { id: null, name: '', description: '', product_ids: [], }
  errores.value = {}
  error.value = ''
...
const cerrarModal = () => {
  ...
  form.value = { id: null, name: '', description: '', product_ids: [], }
}
