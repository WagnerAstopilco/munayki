      ...
      <AdminModal :show="mostrarModal" :on-close="cerrarModal">
        ...
        <form @submit.prevent="guardarProducto">
          <div class="mb-2" v-for="(label, field) in campos" :key="field">
            ...
            <input v-if="['name','price_PEN','price_USD','stock','number_of_days','number_of_nights','number_of_people'].includes(field)"
                   v-model="form[field]" type="text" class="form-control" />
            <textarea v-else-if="field === 'description'" v-model="form.description" class="form-control" rows="2" />
          </div>
  
          <div class="mb-2">
            <label class="form-label">Destino</label>
            <Multiselect v-model="form.destino_id" :options="destinos || []" :track-by="'id'" :label="'name'" placeholder="Destino" />
          </div>
  
          <div class="mb-2">
            <label class="form-label">Categoría</label>
            <Multiselect v-model="form.category_id" :options="categorias || []" :track-by="'id'" :label="'name'" placeholder="Categoría" />
          </div>
  
          <div class="mb-2">
            <label class="form-label">Actividades</label>
            <Multiselect v-model="form.activities" :options="actividades || []" :multiple="true" :track-by="'id'" :label="'name'" placeholder="Selecciona actividades" />
          </div>
          ...
          <div class="mb-2">
            ...
            <div class="mt-2 d-flex flex-wrap">
              <img v-for="img in imagenesPreview" :key="img.name" :src="img.url" class="img-thumbnail me-1 mb-1" style="width: 60px; height: 60px;" />
            </div>
          </div>
  ...
  const abrirFormulario = (producto = null) => {
    if (producto) {
      ...
      Object.assign(form.value, {
        id: null, name: '', description: '', price_PEN: '', price_USD: '', stock: 1,
        number_of_days: 1, number_of_nights: 1, number_of_people: 1,
        destino_id: null, category_id: null, activities: []
      })
    }
    imagenes.value = []
