<template>
  <div class="admin-table-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
      <input v-model="filtro" type="text" class="form-control w-100 w-md-25" placeholder="Buscar...">
      <div class="d-flex gap-2">
        <span class="text-white small">Total: {{ datosFiltrados.length }}</span>
        <button v-if="exportable" class="btn btn-sm btn-outline-light" @click="exportarExcel">
    ...
    <div class="table-responsive rounded overflow-hidden">
      <table class="table table-hover table-bordered table-custom">
        ...
        <tbody>
          <template v-for="item in paginados" :key="item.id">
    ...
    <nav class="mt-3">
      <ul class="pagination justify-content-center">
        <li class="page-item" :class="{ disabled: pagina === 1 }">
          <a class="page-link" @click="pagina > 1 && pagina--">Anterior</a>
        </li>
        <li v-for="n in totalPaginas" :key="n" :class="['page-item', { active: pagina === n }]">
          <a class="page-link" @click="pagina = n">{{ n }}</a>
        </li>
        <li class="page-item" :class="{ disabled: pagina === totalPaginas }">
          <a class="page-link" @click="pagina < totalPaginas && pagina++">Siguiente</a>
        </li>
      </ul>
    </nav>
  </div>
